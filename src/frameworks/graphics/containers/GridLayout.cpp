...
std::vector<float> GridLayout::calculateSizes(const DimensionVector& dimensions, 
    ...
    // First pass: handle fixed sizes and count relative/auto
    ...
    // Second pass: handle relative sizes
    if (totalRelative > 0) {
        float spaceForRelative = remainingSpace * (autoCount > 0 ? 0.8f : 1.0f);
        for (size_t i = 0; i < dimensions.size(); ++i) {
            const auto& dim = dimensions[i];
            if (dim.mode == Dimension::Mode::Relative) {
    ...
    // Third pass: handle auto sizes
    if (autoCount > 0) {
        float autoSize = remainingSpace / autoCount;
        for (size_t i = 0; i < dimensions.size(); ++i) {
            if (dimensions[i].mode == Dimension::Mode::Auto) {
