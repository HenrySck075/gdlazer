...
class FontStore //: TextureStore, ITexturedGlyphLookupStore
{
  ...
  private:
    ...
    std::map<std::tuple<std::string, char>, CCSprite*> namespacedGlyphCache = {};
  ...
  FontStore()//IRenderer renderer, IResourceStore<TextureUpload> store = null, float scaleAdjust = 100)
  ...
  FontStore(IRenderer renderer, IResourceStore<TextureUpload> store = null, float scaleAdjust = 100, TextureFilteringMode minFilterMode = TextureFilteringMode.Linear)
    : this(renderer, store, scaleAdjust, true, filteringMode: minFilterMode)
  {
  }
  */
public:
  FontStore()//IRenderer renderer, IResourceStore<TextureUpload> store = null, float scaleAdjust = 100, bool useAtlas = false, Storage cacheStorage = null, TextureFilteringMode filteringMode = TextureFilteringMode.Linear)
  ...
  /*
  void AddTextureSource(IResourceStore<TextureUpload> store)
  {
